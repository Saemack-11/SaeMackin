...
const STYLES=['More slang','Less slang','Shorter','More detailed','More playful','More serious','More flirty','More direct','More compassionate','Add humor','Remove emotion','Voice-message style'];
const LEVELS=['Soft','Calm','Balanced','Bold','Unfiltered Sae'];
const PRINCIPLES=[['Confident Observation','Make an intelligent observation and give the other person room to confirm it. Feeling seen is stronger than feeling chased.'],['Controlled Heat','Acknowledge attraction, add one clean dose of sexual humor, then return to grounded intention.'],['Match Investment','Do not send five paragraphs to somebody giving five-word replies unless the moment genuinely calls for depth.'],['Listener Before Fixer','Figure out whether they want comfort, advice, a solution or simply somebody present.'],['Comedian’s Composure','Read the room, handle the heckler without losing character, then keep the show moving.'],['Exit With Dignity','Not every conversation needs to be rescued. Sometimes composure means no longer forcing it.']];
...
$('#intensity').oninput=e=>{$('#intensityText').textContent=LEVELS[e.target.value-1];renderMode()};
function inferTone(text){const t=text.toLowerCase();if(/[?]{2,}|why|confused|what do you mean/.test(t))return['Seeking clarity','They may be testing whether you understand the real issue.','Answer the question before adding personality.'];if(/sorry|hurt|sad|tired|overwhelmed|stress|cry/.test(t))return['Emotionally open','The message carries vulnerability or fatigue.','Lead with presence; advice comes second.'];if(/lol|😂|lmao|haha|play/.test(t))return['Playful and receptive','They are giving you room to joke and build rhythm.','Match the humor without trying to perform too hard.'];if(/no|stop|leave|done|mad|angry|disrespect/.test(t))return['Defensive or firm','The temperature is elevated and the wrong line could escalate it.','Keep the point sharp and the ego quiet.'];if(/thank|appreciate|love|glad|yes|would love/.test(t))return['Warm and open','They are rewarding the tone and showing willingness to continue.','Receive the warmth and leave space for another exchange.'];return['Neutral but readable','There is enough information to respond, but not enough to over-assume.','Stay natural, ask one useful question and avoid overexplaining.']}
function opener(p){return {'Romantic Sae':'I appreciate you being real with me. That means more than somebody just saying the right thing.','Flirtatious Sae':'See, that right there just told me a little more about you 😏','Business Sae':'Thank you for the update. I want to make sure we handle this clearly and efficiently.','Listener Sae':'I’m listening. You don’t have to clean it up or make it sound easier than it is.','Frustrated Sae':'I’m not trying to argue, but I do need to be honest about what bothered me.','Logical Angry Sae':'I’m going to keep this direct and focused on the facts.','Toxic Sae':'Oh, so that’s the energy we bringing today? Bet 😂','Motivator Sae':'You’ve already survived the part that was supposed to break you.','Comedian Sae':'See, now you done gave me material 😂','Advisor Sae':'Let’s slow it down and separate what happened from what it made you feel.','Negotiator Sae':'I believe there’s a fair outcome here that respects both sides.','Grounded Sae':'I understand where you’re coming from, and I want to answer without reacting carelessly.'}[p]}
...
function renderStats(){const ratings=Object.values(store.ratings).reduce((a,b)=>a+b,0),favorite=Object.entries(store.usage).sort((a,b)=>b[1]-a[1])[0]?.[0]||'Not enough data';$('#stats').innerHTML=`<div class="stat"><b>${store.generations}</b><span>Replies generated</span></div><div class="stat"><b>${store.saved.length}</b><span>Saved lessons</span></div><div class="stat"><b>${ratings}</b><span>Tone ratings</span></div><div class="stat"><b style="font-size:1rem">${favorite}</b><span>Most-used Sae</span></div>`}
$('#randomPersona').onclick=()=>{store.persona=PERSONAS[Math.floor(Math.random()*PERSONAS.length)].name;localStorage.sm_persona=store.persona;renderPersonas();renderMode();toast(store.persona)};$('#resetBtn').onclick=()=>{$('#relationship').selectedIndex=0;$('#goal').selectedIndex=0;$('#intensity').value=3;$('#intensityText').textContent='Balanced';store.styles.clear();localStorage.sm_styles='[]';renderChips();renderMode();toast('Studio reset')};
function toast(t){const x=$('#toast');x.textContent=t;x.classList.add('show');setTimeout(()=>x.classList.remove('show'),1500)}let deferred;addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferred=e;$('#installBtn').classList.remove('hidden')});$('#installBtn').onclick=async()=>{if(!deferred)return;deferred.prompt();await deferred.userChoice;deferred=null;$('#installBtn').classList.add('hidden')};if('serviceWorker'in navigator)addEventListener('load',()=>navigator.serviceWorker.register('sw.js'));
renderPersonas();renderMode();renderChips();renderPrinciples();renderSaved();renderStats();
